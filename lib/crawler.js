...
Crawler.prototype.init = function init (options) {
    ...
    // Generate phantom instance when options.phantom is setting true.
    if (options.phantom) {
        Phantom.create().then(function (phInstance) {
            self.phInstance = phInstance;
        }).catch(function (error) {
            console.error('phantom instance create error: ', error);
        });
    }
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    // Do request by Phantom if options.phantom is true.
    if (options.phantom) {
        return (function () {
            if (self.phInstance) {
                return self.phInstance.createPage();
            } else {
            ...
            return page.open(options.uri).then(function () {
                return Promise.all([
